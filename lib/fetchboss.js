  ...
  */
  setPriority (pri, item) {
    ...
    if (obj === undefined) {
      ...
      this.maybeStale.delete(obj)
    }
    obj.priority = pri
    if (pri < 0.000001) this.items.delete(key)
  }
  ...
  rethink () {
    ...
    const done = () => {
      ...
      // zero out everything still in maybeStale
      for (let i of this.maybeStale) {
        this.setPriority(0, i)
      }
    ...
    this.maybeStale = new Set(this.items.values())
